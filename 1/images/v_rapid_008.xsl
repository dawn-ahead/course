...
<xsl:template match="params">
	...
	<xsl:variable name="pointer-color-custom-fixed">
		<xsl:call-template name="fix-color">
			<xsl:with-param name="color" select="pointer_color_custom"/>
		</xsl:call-template>
	</xsl:variable>
	...
	<xsl:variable name="main.iItemToPreview">
		<xsl:choose>
			<xsl:when test="$main.arItemsToPreview=''">1</xsl:when>
	...
	<xsl:variable name="main.sDescRadiusString">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">0px 0px <xsl:value-of select="desc_radius"/>px <xsl:value-of select="desc_radius"/>px</xsl:when>
			<xsl:when test="desc_position='left'"><xsl:value-of select="desc_radius"/>px 0 0 <xsl:value-of select="desc_radius"/>px</xsl:when>
			<xsl:otherwise>0px <xsl:value-of select="desc_radius"/>px <xsl:value-of select="desc_radius"/>px 0</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sDescCoordsCSS">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">left: 0; bottom: 0;</xsl:when>
			<xsl:when test="desc_position='left'">left: 0; top: 0;</xsl:when>
			<xsl:otherwise>right: 0; top: 0;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sDescSizeCSS">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">width: <xsl:value-of select="number($main.nInnerWidth) - 2*number(desc_border_width)"/>px; height: <xsl:value-of select="floor(0.5*number($main.nTabBlockHeight)) - 2*number(desc_border_width)"/>px;</xsl:when>
			<xsl:otherwise>width: <xsl:value-of select="number($main.nInnerWidth) - number($main.nTabBlockHeight) - 2*number(desc_border_width)"/>px; height: <xsl:value-of select="number($main.nTabBlockHeight) - 2*number(desc_border_width)"/>px;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sImgContainerCoordsCSS">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">left: 0; top: 0;</xsl:when>
			<xsl:when test="desc_position='left'">right: 0; top: 0;</xsl:when>
			<xsl:otherwise>left: 0; top: 0;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sImgContainerSizeCSS">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">width: <xsl:value-of select="$main.nInnerWidth"/>px; height: <xsl:value-of select="floor(0.5*number($main.nTabBlockHeight))"/>px;</xsl:when>
			<xsl:otherwise>width: <xsl:value-of select="0.5*number($main.nInnerWidth)"/>px; height: <xsl:value-of select="$main.nTabBlockHeight"/>px;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sImgAlignCSS">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">text-align: center;</xsl:when>
			<xsl:when test="desc_position='left'">text-align: right;</xsl:when>
			<xsl:otherwise>text-align: left;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>

	<xsl:variable name="main.sPointerColor1">
		<xsl:call-template name="define-color">
			<xsl:with-param name="sProfile" select="pointer_color_scheme"/>
			<xsl:with-param name="sColorType">color1</xsl:with-param>
			<xsl:with-param name="sCustom" select="$pointer-color-custom-fixed"/>
			<xsl:with-param name="sCustomGradient" select="$pointer-color-custom-fixed"/>
			<xsl:with-param name="sCustomBorder" select="$pointer-color-custom-fixed"/>
		</xsl:call-template>
	</xsl:variable>
	<xsl:variable name="main.sPointerColorBorder">
		<xsl:call-template name="define-color">
			<xsl:with-param name="sProfile" select="pointer_color_scheme"/>
			<xsl:with-param name="sColorType">border</xsl:with-param>
			<xsl:with-param name="sCustom" select="$pointer-color-custom-fixed"/>
			<xsl:with-param name="sCustomGradient" select="$pointer-color-custom-fixed"/>
			<xsl:with-param name="sCustomBorder">
				<xsl:call-template name="getcolor">
					<xsl:with-param name="color.base" select="$pointer-color-custom-fixed"/>
					<xsl:with-param name="color.base.type">color1</xsl:with-param>
	...
	<xsl:variable name="main.nPointerBorderWidth">2</xsl:variable>
	<xsl:variable name="main.nPointerArrowLength">32</xsl:variable>
	<xsl:variable name="main.nPointerHalfWidth">10</xsl:variable>
	<xsl:variable name="main.nPointerLineSize">8</xsl:variable>
	<xsl:variable name="main.nPointerLineWidth">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'"><xsl:value-of select="$main.nPointerLineSize"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="number($main.nInnerWidth) - number($main.nPointerLineSize)"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.nPointerLineHeight">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'"><xsl:value-of select="number($main.nInnerWidth) - number($main.nPointerLineSize)"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="$main.nPointerLineSize"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>

	<xsl:variable name="main.nPointerBGArrowLength" select="number($main.nPointerArrowLength) + 2*number($main.nPointerBorderWidth)"/>
	<xsl:variable name="main.nPointerBGArrowHalfWidth" select="number($main.nPointerHalfWidth) + number($main.nPointerBorderWidth)"/>
	<xsl:variable name="main.nPointerWidth">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'"><xsl:value-of select="2*number($main.nPointerBGArrowHalfWidth)"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="$main.nInnerWidth"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.nPointerHeight">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'"><xsl:value-of select="$main.nTabBlockHeight"/></xsl:when>
			<xsl:otherwise><xsl:value-of select="2*number($main.nPointerBGArrowHalfWidth)"/></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sPointerArrowMarginCSS">
		<xsl:choose>
			<xsl:when test="desc_position='right'">margin: <xsl:value-of select="$main.nPointerBorderWidth"/>px 0 0 <xsl:value-of select="$main.nPointerBorderWidth"/>px;</xsl:when>
			<xsl:when test="desc_position='bottom'">margin: <xsl:value-of select="$main.nPointerBorderWidth"/>px 0 0 <xsl:value-of select="$main.nPointerBorderWidth"/>px;</xsl:when>
			<xsl:otherwise>margin: <xsl:value-of select="$main.nPointerBorderWidth"/>px <xsl:value-of select="$main.nPointerBorderWidth"/>px 0 0;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sPointerLineMarginCSS">
		<xsl:choose>
			<xsl:when test="desc_position='right'">margin: <xsl:value-of select="number($main.nPointerBGArrowHalfWidth) - 0.5*number($main.nPointerLineSize) - number($main.nPointerBorderWidth)"/>px 0 0 <xsl:value-of select="number($main.nPointerBGArrowLength) - number($main.nPointerBorderWidth)"/>px;</xsl:when>
			<xsl:when test="desc_position='bottom'">margin: <xsl:value-of select="number($main.nPointerBGArrowLength) - number($main.nPointerBorderWidth)"/>px 0 0 <xsl:value-of select="number($main.nPointerBGArrowHalfWidth) - 0.5*number($main.nPointerLineSize) - number($main.nPointerBorderWidth)"/>px;</xsl:when>
			<xsl:otherwise>margin: <xsl:value-of select="number($main.nPointerBGArrowHalfWidth) - 0.5*number($main.nPointerLineSize) - number($main.nPointerBorderWidth)"/>px <xsl:value-of select="number($main.nPointerBGArrowLength) - number($main.nPointerBorderWidth)"/>px 0 0;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sPointerLineCoordsCSS">
		<xsl:choose>
			<xsl:when test="desc_position='right'">left: 0; top: 0;</xsl:when>
			<xsl:when test="desc_position='bottom'">left: 0; top: 0;</xsl:when>
			<xsl:otherwise>right: 0; top: 0;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	<xsl:variable name="main.sPointerLineBorderCSS">
		<xsl:choose>
			<xsl:when test="desc_position='bottom'">border-left: solid <xsl:value-of select="$main.nPointerBorderWidth"/>px #fff; border-right: solid <xsl:value-of select="$main.nPointerBorderWidth"/>px #fff;</xsl:when>
			<xsl:otherwise>border-top: solid <xsl:value-of select="$main.nPointerBorderWidth"/>px #fff; border-bottom: solid <xsl:value-of select="$main.nPointerBorderWidth"/>px #fff;</xsl:otherwise>
		</xsl:choose>
	</xsl:variable>

	<div>
		<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_IMGSIZE</xsl:attribute>
		<xsl:attribute name="class">cl-img-adjust</xsl:attribute>
		<xsl:attribute name="style">position: absolute; top: 0px; left: 0px; visibility: hidden;</xsl:attribute>
		<xsl:choose>
			<xsl:when test="string-length(picture_file)=0">
				<img width="1" height="1">
					<xsl:attribute name="src">../courseimages/1blank.gif</xsl:attribute>
					<xsl:attribute name="onload">CLO['<xsl:value-of select="$objectID"/>'].CallMethod({ sMethod: 'Adjust', oMethodArgs: { elem: this } });</xsl:attribute>
				</img>
			</xsl:when>
			<xsl:otherwise>
				<img>
					<xsl:attribute name="src"><xsl:value-of select="picture_file"/></xsl:attribute>
					<xsl:attribute name="onload">CLO['<xsl:value-of select="$objectID"/>'].CallMethod({ sMethod: 'Adjust', oMethodArgs: { elem: this } });</xsl:attribute>
				</img>
			</xsl:otherwise>
		</xsl:choose>
	...
	<div class="style-custom" style="display: none">
		...
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-img-container</xsl:attribute>
			<span class="rule-static"><xsl:value-of select="$main.sImgContainerCoordsCSS"/> <xsl:value-of select="$main.sImgContainerSizeCSS"/> <xsl:value-of select="$main.sImgAlignCSS"/></span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-desc</xsl:attribute>
			<span class="rule-static"><xsl:value-of select="$main.sDescCoordsCSS"/> <xsl:value-of select="$main.sDescSizeCSS"/> background-color: <xsl:value-of select="$main.sDescColor1"/>; border-width: <xsl:value-of select="desc_border_width"/>px; border-color: <xsl:value-of select="$main.sDescBorderColor"/>;</span>
			<span class="rule-dynamic">
				<xsl:attribute name="data-type">border-radius</xsl:attribute>
				<xsl:attribute name="data-value"><xsl:value-of select="$main.sDescRadiusString"/></xsl:attribute>
			</span>
		...
		<div class="rule">
			...
			<span class="rule-static">margin: <xsl:value-of select="desc_padding"/>px; height: <xsl:value-of select="number($main.nTabBlockHeight) - 2*number(desc_padding)"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-pointer</xsl:attribute>
			<span class="rule-static">width: <xsl:value-of select="$main.nPointerWidth"/>px; height: <xsl:value-of select="$main.nPointerHeight"/>px;</span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-pointer-arrow</xsl:attribute>
			<span class="rule-static"><xsl:value-of select="$main.sPointerArrowMarginCSS"/><xsl:choose><xsl:when test="desc_position='right'">left: 0; top: 0; border-width: <xsl:value-of select="$main.nPointerHalfWidth"/>px <xsl:value-of select="$main.nPointerArrowLength"/>px <xsl:value-of select="$main.nPointerHalfWidth"/>px 0; border-color: transparent <xsl:value-of select="$main.sPointerColorBorder"/> transparent transparent;</xsl:when><xsl:when test="desc_position='bottom'">left: 0; top: 0; border-width: 0 <xsl:value-of select="$main.nPointerHalfWidth"/>px <xsl:value-of select="$main.nPointerArrowLength"/>px <xsl:value-of select="$main.nPointerHalfWidth"/>px; border-color: transparent transparent <xsl:value-of select="$main.sPointerColorBorder"/> transparent;</xsl:when><xsl:otherwise>right: 0; top: 0; border-width: <xsl:value-of select="$main.nPointerHalfWidth"/>px 0 <xsl:value-of select="$main.nPointerHalfWidth"/>px <xsl:value-of select="$main.nPointerArrowLength"/>px; border-color: transparent transparent transparent <xsl:value-of select="$main.sPointerColorBorder"/>;</xsl:otherwise></xsl:choose></span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-pointer-bg-arrow</xsl:attribute>
			<span class="rule-static">margin: 0; <xsl:choose><xsl:when test="desc_position='right'">left: 0; top: 0; border-width: <xsl:value-of select="$main.nPointerBGArrowHalfWidth"/>px <xsl:value-of select="$main.nPointerBGArrowLength"/>px <xsl:value-of select="$main.nPointerBGArrowHalfWidth"/>px 0; border-color: transparent #fff transparent transparent;</xsl:when><xsl:when test="desc_position='bottom'">left: 0; top: 0; border-width: 0 <xsl:value-of select="$main.nPointerBGArrowHalfWidth"/>px <xsl:value-of select="$main.nPointerBGArrowLength"/>px <xsl:value-of select="$main.nPointerBGArrowHalfWidth"/>px; border-color: transparent transparent #fff transparent;</xsl:when><xsl:otherwise>right: 0; top: 0; border-width: <xsl:value-of select="$main.nPointerBGArrowHalfWidth"/>px 0 <xsl:value-of select="$main.nPointerBGArrowHalfWidth"/>px <xsl:value-of select="$main.nPointerBGArrowLength"/>px; border-color: transparent transparent transparent #fff;</xsl:otherwise></xsl:choose></span>
		</div>
		<div class="rule">
			<xsl:attribute name="data-name">#<xsl:value-of select="$objectID"/> .cl-pointer-line</xsl:attribute>
			<span class="rule-static"><xsl:value-of select="$main.sPointerLineCoordsCSS"/> <xsl:value-of select="$main.sPointerLineMarginCSS"/> <xsl:value-of select="$main.sPointerLineBorderCSS"/> width: <xsl:value-of select="$main.nPointerLineWidth"/>px; height: <xsl:value-of select="$main.nPointerLineHeight"/>px; background-color: <xsl:value-of select="$main.sPointerColorBorder"/>;</span>
		</div>
	</div>
	<div>
		...
		<div>
			...
			<div>
				...
				<div>
					<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_IMG_CONTAINER</xsl:attribute>
					<xsl:attribute name="class">cl-img-container</xsl:attribute>
					<xsl:choose>
						<xsl:when test="string-length(picture_file)=0">
							<table cellpadding="0" cellspacing="0" border="0">
								<xsl:attribute name="class">cl-no-img-table</xsl:attribute>
								<tr>
									<td>
										<xsl:attribute name="class">cl-no-img-table-td</xsl:attribute>
										<xsl:value-of select="hidden.notdefined"/>
									</td>
								</tr>
							</table>
						</xsl:when>
						<xsl:otherwise>
							<img>
								<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_IMG</xsl:attribute>
								<xsl:attribute name="class">cl-img</xsl:attribute>
								<xsl:attribute name="src"><xsl:value-of select="picture_file"/></xsl:attribute>
							</img>
						</xsl:otherwise>
					</xsl:choose>
				...
				<div>
					<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_DESC</xsl:attribute>
					<xsl:attribute name="class">cl-desc</xsl:attribute>
					<xsl:for-each select="elements/item">
						<div>
							<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_DESC_TEXT_<xsl:value-of select="position()"/></xsl:attribute>
							<xsl:attribute name="class">cl-desc-text</xsl:attribute>
							<xsl:attribute name="style"><xsl:if test="number($main.iItemToPreview)!=position()">display: none;</xsl:if></xsl:attribute>
				...
				<div>
					...
					<xsl:attribute name="class">cl-pointer</xsl:attribute>
					<div>
						<xsl:attribute name="class">cl-pointer-bg-arrow</xsl:attribute>
						&#160;
					</div>
					<div>
						<xsl:attribute name="class">cl-pointer-arrow</xsl:attribute>
						&#160;
					</div>
					<div>
						<xsl:attribute name="id"><xsl:value-of select="$objectID"/>_POINTER_LINE</xsl:attribute>
						<xsl:attribute name="class">cl-pointer-line</xsl:attribute>
						&#160;
					</div>
				</div>
			</div>
