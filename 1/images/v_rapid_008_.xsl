...
<xsl:template match="params">
	...
	<style>
		...
		.cl-img-container { <xsl:value-of select="$main.sImgContainerCoordsCSS"/> <xsl:value-of select="$main.sImgContainerSizeCSS"/> <xsl:value-of select="$main.sImgAlignCSS"/> }
		.cl-desc { <xsl:value-of select="$main.sDescCoordsCSS"/> <xsl:value-of select="$main.sDescSizeCSS"/> background-color: <xsl:value-of select="$main.sDescColor1"/>; border-width: <xsl:value-of select="desc_border_width"/>px; border-color: <xsl:value-of select="$main.sDescBorderColor"/>; border-radius: <xsl:value-of select="$main.sDescRadiusString"/>; <xsl:if test="$main.sDescColor1!=$main.sDescColor2">background-image: linear-gradient(<xsl:value-of select="desc_bg_grad_angle"/>deg, <xsl:value-of select="$main.sDescColor1"/>, <xsl:value-of select="$main.sDescColor2"/>);</xsl:if> border-width: <xsl:value-of select="desc_border_width"/>px; <xsl:if test="shadow_strength!='none'">box-shadow: <xsl:value-of select="$main.sShadowString"/>;</xsl:if> }
		.cl-desc-text { margin: <xsl:value-of select="desc_padding"/>px; height: <xsl:value-of select="number($main.nTabBlockHeight) - 2*number(desc_padding)"/>px; }
	</style>
	<div>
		<xsl:attribute name="class">clo-v_rapid_008</xsl:attribute>
		<div>
						...
						<xsl:choose>
							...
							<xsl:otherwise>
								<img>
									...
									<xsl:attribute name="src"><xsl:value-of select="substring-before($moduleImagesFolder,'images\')"/><xsl:value-of select="translate(picture_file,'/','\')"/></xsl:attribute>
								</img>
							</xsl:otherwise>
						</xsl:choose>
					</div>
	...
	function Adjust()
	{
		try
		{
			var sLayout = '<xsl:value-of select="desc_position"/>';
			var sScaling = '<xsl:value-of select="picture_scaling"/>';
			var oImg = document.getElementById('<xsl:value-of select="$objectID"/>_IMG');
			if(oImg==null) return false;
			var oImgContainer = document.getElementById('<xsl:value-of select="$objectID"/>_IMG_CONTAINER');
			var oDesc = document.getElementById('<xsl:value-of select="$objectID"/>_DESC');
			var aDescText = document.getElementsByClassName('cl-desc-text');
			var iFullW = +<xsl:value-of select="$main.nInnerWidth"/>;
			var iFullH = +<xsl:value-of select="$main.nTabBlockHeight"/>;
			var iPadding = +<xsl:value-of select="bg_padding"/>;
			var iBW = +<xsl:value-of select="desc_border_width"/>;
			var iDescPadding = +<xsl:value-of select="desc_padding"/>;
			var iRadius = +<xsl:value-of select="desc_radius"/>;
			var iContW = oImgContainer.offsetWidth;
			var iContH = oImgContainer.offsetHeight;
			...
			switch(sLayout)
			{
				case "left":
				{
					oDesc.style.height = (iFullH - 2*iBW) + "px";
					iCnt = 0;
					while(aDescText[iCnt]!=null)
					{
						aDescText[iCnt].style.height = (iFullH - 2*iBW - 2*iDescPadding) + "px";
						iCnt++;
					}
					oDesc.style.left = 0;
					oDesc.style.top = 0;
					switch(sScaling)
					{
						case "none":
						{
							iNewW = iW;
							iNewH = iH;
							oImgContainer.style.top = 0;
							oImgContainer.style.right = 0;
							oImgContainer.style.width = iNewW + "px";
							break;
						}
						case "hw":
						{
							...
							oImg.style.width = iNewW + "px";
							oImg.style.height = iNewH + "px";
							oImg.style.marginTop = Math.round(0.5*(iFullH - iNewH)) + "px";
							oImgContainer.style.top = 0;
							oImgContainer.style.right = 0;
							oImgContainer.style.width = iNewW + "px";
							break;
						...
						case "hh":
						{
							...
							oImg.style.width = iNewW + "px";
							oImg.style.height = iNewH + "px";
							oImg.style.marginTop = Math.round(0.25*iFullH) + "px";
							oImgContainer.style.top = 0;
							oImgContainer.style.right = 0;
							oImgContainer.style.width = iNewW + "px";
							break;
					...
					if(Math.abs(iDescW-100)!=(iDescW-100))
					{
						iDescW = 100;
						oDesc.style.left = (iFullW - iNewW - iPadding - iDescW) + "px";
					}
					oDesc.style.width = iDescW + "px";
					iCnt = 0;
					while(aDescText[iCnt]!=null)
					{
						aDescText[iCnt].style.width = (iDescW - 2*iDescPadding) + "px";
						iCnt++;
					}
					break;
				}
				case "right":
				{
					...
					{
						iDescW = 100;
						oDesc.style.right = (iFullW - iNewW - iPadding - iDescW) + "px";
					}
					oDesc.style.width = iDescW + "px";
					iCnt = 0;
					while(aDescText[iCnt]!=null)
				...
				case "bottom":
				{
					oDesc.style.width = (iFullW - 2*iBW) + "px";
					oDesc.style.left = 0;
					oDesc.style.bottom = 0;
					iCnt = 0;
					while(aDescText[iCnt]!=null)
					{
						aDescText[iCnt].style.width = (iFullW - 2*iBW - 2*iDescPadding) + "px";
						iCnt++;
					}
					switch(sScaling)
					{
						case "w":
						{
							...
							oImgContainer.style.top = 0;
							oImgContainer.style.right = 0;
							oImgContainer.style.height = ((Math.abs(iFullH-iNewH)==(iFullH-iNewH)) ? iFullH : iNewH) + "px";
							break;
						}
						case "none":
						{
						...
						default:
						{
							...
							oImgContainer.style.top = 0;
							oImgContainer.style.right = 0;
							oImgContainer.style.height = iNewH + "px";
							break;
						}
					}
					iDescH = iFullH - iNewH - iPadding;
					if(Math.abs(iDescH-100)!=(iDescH-100))
					{
						iDescH = 100;
						oDesc.style.bottom = (iFullH - iNewH - iPadding - iDescH) + "px";
					}
					oDesc.style.height = iDescH + "px";
					iCnt = 0;
					while(aDescText[iCnt]!=null)
					{
						aDescText[iCnt].style.height = (iDescH - 2*iDescPadding) + "px";
						iCnt++;
					}
					break;
				}
